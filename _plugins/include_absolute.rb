module Jekyll
  module Tags
    ...
    class IncludeAbsoluteTag < Liquid::Tag
      ...
      !x
      VARIABLE_SYNTAX = %r!
        (?<variable>[^{]*(\{\{\s*[\w\-\.\u4e00-\u9fa5]+\s*(\|.*)?\}\}[^\s{}]*)+)
        (?<params>.*)
      !mx

      FULL_VALID_SYNTAX = %r!\A\s*(?:#{VALID_SYNTAX}(?=\s|\z)\s*)*\z!
      VALID_FILENAME_CHARS = %r!^[\w/\.\-\u4e00-\u9fa5]+$!
...
MSG
      ...
      def render(context)
        ...
        file = context[@file] || render_variable(context) || @file
        # strip leading and trailing quote's
        # file = file.gsub!(/\A'|'\Z/, '')
        file.gsub!(/.#.*/,'')
        #puts('111'+file)#.class.to_s
      ...
      def read_file(file, context)
        (File.read(file, file_read_opts(context))[0,250]).gsub(/<\/?[^>]*>/, '').gsub(/\n\n+/, "\n").gsub(/^\n|\n$/, '').gsub(/</, "").gsub(/>/, "")#CGI.escape()
      end
